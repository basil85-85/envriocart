      ...
      try {
            ...
            Object.entries(sizes).forEach(([size, quantity]) => {
                  ...
                  if (processedSizes.hasOwnProperty(upperSize)) {
                        processedSizes[upperSize] = Math.max(
                              0,
                              parseInt(quantity) || 0
                        )
                  }
            ...
            const totalQuantity = Object.values(processedSizes).reduce(
                  (sum, qty) => sum + qty,
                  0
            )
            if (totalQuantity === 0) {
                  return res.status(400).json({
                        success: false,
                        message:
                              'At least one size must have a quantity greater than 0',
                  })
            }
