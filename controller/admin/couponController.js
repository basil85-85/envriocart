...
const addingCoupon = async (req, res) => {
      try {
            ...
            return res
                  .status(500)
                  .json({ success: false, messgae: `server error${error} ` })
      }
}
      ...
      try {
            ...
            if (!coupon) {
                  return res
                        ...
                        .json({
                              success: true,
                              message: 'coupon is not founding',
                        })
            }
            let newSate = ''
            if (coupon.status === 'active') {
                  ...
                  return res
                        ...
                        .json({
                              success: false,
                              message: "U can't change the state",
                        })
            }
            const updateState = await Coupon.findByIdAndUpdate(
                  couponId,
                  { status: newSate },
                  { new: true }
            )
            ...
            return res
                  ...
                  .json({
                        success: true,
                        message: 'sucessfully chanegd the state',
                        coupon,
                  })
      } catch (error) {
            ...
            return res
                  .status(500)
                  .json({ success: false, messgae: 'server error occur' })
      }
}
...
const deleteCoupon = async (req, res) => {
      try {
            ...
            return res
                  ...
                  .json({
                        success: true,
                        message: 'coupon is delected sucessfully',
                  })
      } catch (error) {
